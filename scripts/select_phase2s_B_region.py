...
def _latest_matching() -> tuple[Path, list[dict]]:
    ...
    if not candidates:
        raise FileNotFoundError("no Phase 2S matched states found")
    path = max(candidates)[1]
    return path, _read_jsonl(path)


def _placement(region, region_index, placement_index, seed):
...
def _state_task(payload):
    row, regions, phase2, phase2s = payload
    ...
    state = free_fingertip_workspace_clouds(
        ...
        phase2s.second_grasp.geometry_seed,
        base_cfg=cfg,
    )
    results = []
    for region_index, region in enumerate(regions):
        for placement_index in range(phase2s.second_grasp.geometry_placements_per_region):
            result = analyze_B_geometry_state(
                *state,
                phase2.resources,
                _placement(region, region_index, placement_index, phase2s.second_grasp.geometry_seed),
            )
...
def main() -> int:
    ...
    if not regions:
        raise RuntimeError("small-B validation produced no positive local region")
    cfg_hash = config_hash([phase2s_path, phase2_path, matched_path, regions_path])
    output = ROOT / phase2s.output_dir / "diagnostics" / "B_region_selection" / cfg_hash[:12]
    output.mkdir(parents=True, exist_ok=True)
    workers = min(
        args.workers or max(1, (os.cpu_count() or 1) // 2), phase2s.state.maximum_workers
    )
    with ProcessPoolExecutor(max_workers=workers) as executor:
        state_results = []
        for index, result in enumerate(
            executor.map(_state_task, ((row, regions, phase2, phase2s) for row in states)), start=1
        ):
            state_results.append(result)
            if index % workers == 0 or index == len(states):
                print(f"Phase 2S B geometry states: {index}/{len(states)}", flush=True)
    ...
    for region in regions:
        ...
        for state_type in ("FINGERTIP", "PALMAR_SECURED"):
            rows = [
                ...
                for result in state["results"] if result["region"] == region["name"]
            ]
            by_type[state_type] = {
        ...
        summaries.append({
            ...
            "groups": by_type,
            "minimum_group_access_fraction": min(
                by_type[group]["access_fraction"] for group in by_type
            ),
            "zero_initial_A_overlap": all(
                by_type[group]["initial_A_overlap_pairs"] == 0 for group in by_type
            ),
        })
    eligible = [
    ...
    selected = max(
        eligible,
        key=lambda row: (
            row["minimum_group_access_fraction"], row["B_only_robustness_fraction"], row["region"]
        ),
    ) if eligible else None
    summary = {
        "status": "PASS" if selected is not None else "PHASE2S_NO_COMMON_B_REGION",
        "selection_basis": "maximize minimum matched-group geometric access; strict half-scale B-only evidence; zero A overlap; no dynamic formal outcomes",
        "regions": summaries,
    ...
    frozen = {
        ...
        "source_half_scale_region": region["name"],
        "source_B_only_profile": region["source_profile"],
        ...
        "geometry_seed_namespace": phase2s.second_grasp.geometry_seed,
        "calibration_seed_namespace": phase2s.second_grasp.calibration_seed,
        "formal_seed_namespace": phase2s.second_grasp.formal_seed,
        "B_only_robustness_fraction": region["B_only_robustness_fraction"],
        "B_only_trials": region["B_only_trials"],
        "B_only_successes": region["B_only_successes"],
    ...
    report = ROOT / "docs" / "PHASE2S_B_DISTRIBUTION_FREEZE.md"
    report.write_text(
        "# Phase 2S common B distribution freeze\n\n"
        "The common region was selected before any Phase 2S formal dynamic outcome. Selection used only "
        "strict half-scale B-only evidence, matched-state geometry, and initial-overlap checks.\n\n"
        f"- Selected local region: `{region['name']}`\n"
        f"- Source strict B-only profile: `{region['source_profile']}`\n"
        f"- xyz bounds [m]: `{json.dumps(region['center_bounds_m'], sort_keys=True)}`\n"
        f"- yaw bounds [rad]: `{json.dumps(region['yaw_bounds_rad'])}`\n"
        f"- B-only evidence: {region['B_only_successes']}/{region['B_only_trials']} ({region['B_only_robustness_fraction']})\n"
        ...
        f"- Seed namespaces: geometry {phase2s.second_grasp.geometry_seed}, calibration {phase2s.second_grasp.calibration_seed}, formal {phase2s.second_grasp.formal_seed}\n"
