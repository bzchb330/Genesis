...
def _write_outputs(run,cfg):
    ...
    if cfg.diagnostic.save_npz:
        ...
        resource_keys=("time","joint_positions","joint_velocities","joint_limits","distance_to_joint_limits","actuator_controls","actuator_control_limits","actuator_saturated","active_fingers","active_finger_count","active_object_fingers","inactive_object_fingers","active_object_finger_count","contact_count","active_contact_count","finger_contact_count","finger_object_contact_count","finger_object_contact_position_world","finger_object_contact_normal_world","finger_object_contact_distance_m","finger_object_normal_force_raw","tactile_contact_flags","tactile_normal_force","object_table_contact","palm_pose","object_position","object_orientation")
...
def run_scripted_grasp(cfg:ConfigBundle,seed:int|None=None,output_dir:str|Path|None=None,render_video:bool|None=None,save_outputs:bool=True,profile_name:str|None=None)->DiagnosticRun:
    ...
    for stage,duration in profile.stage_durations_seconds.items():
        ...
        for k in range(steps):
            ...
            desired=(1-alpha)*start+alpha*target; q,qvel=hand_state(data,indices); torque=controller.torque(desired,q,qvel); data.ctrl[indices.actuator_ids]=torque
            ...
            object_table_contact=any({record.geom1_name,record.geom2_name}=={object_geom,"table"} for record in contacts); saturated=np.isclose(np.abs(torque),cfg.task.torque_limit,rtol=0,atol=1e-10)
