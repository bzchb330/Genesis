...
class TaskConfig:
    ...
    residual_limit: float; observations: dict[str, bool]; tactile_normalization: float
...
@dataclass(frozen=True)
class ConfigBundle:
    hand: HandConfig; scene: SceneConfig; task: TaskConfig; train: TrainConfig
...
def load_configs(config_dir: str | Path | None = None) -> ConfigBundle:
    ...
    h, s, t, tr = (_read(d / n) for n in ("hand_allegro.yaml", "scene_two_object.yaml", "task_sequential.yaml", "train_ppo.yaml"))
    s["objects"] = [ObjectConfig(**x) for x in s["objects"]]
    return ConfigBundle(HandConfig(**h), SceneConfig(**s), TaskConfig(**t), TrainConfig(**tr))
