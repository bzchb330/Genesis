...
from ..control import JointImpedanceController, residual_torque
...
class SequentialGraspEnv(gym.Env):
    ...
    def reset(self,*,seed=None,options=None):
        ...
        self.phase=Phase.APPROACH_A; self.steps=0; self.desired_q=self.data.qpos[:self.cfg.hand.dof_count].copy()
        obs,_=build_observation(self.model,self.data,self.cfg,self.phase); return obs,{"phase":int(self.phase)}
    def step(self,action):
        action=np.asarray(action,dtype=np.float64); n=self.cfg.hand.dof_count
        base=self.controller.torque(self.desired_q,self.data.qpos[:n],self.data.qvel[:n])
        ...
        self.steps+=1; obs,tactile=build_observation(self.model,self.data,self.cfg,self.phase)
        new_phase=update_phase(self.phase,tactile,self.cfg)
        if new_phase != self.phase: obs,_=build_observation(self.model,self.data,self.cfg,new_phase)
        ...
        return obs,reward,terminated,truncated,{"phase":int(self.phase),"reward_terms":terms,"failure_reason":reason}
    def render(self):
